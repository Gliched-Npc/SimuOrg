...
def clean_dataframe(df: pd.DataFrame) -> tuple[pd.DataFrame, int, int, dict, list[str]]:
    ...
    if age_nulls > 0:
        cleaning_audit.append(f"Age: Filled {age_nulls} missing values with median ({_median_age:.0f})")
    ...
    if mi_nulls > 0:
        cleaning_audit.append(f"MonthlyIncome: Filled {mi_nulls} missing values with median (${_med_income:,.0f})")
    ...
    if 'PerformanceRating' in df.columns:
        ...
        _perf_mode = int(df['PerformanceRating'].dropna().mode().iloc[0]) if df['PerformanceRating'].notna().any() else 3
        df['PerformanceRating'] = df['PerformanceRating'].round(0).fillna(_perf_mode).astype(int)
        if pr_nulls > 0:
            cleaning_audit.append(f"PerformanceRating: Filled {pr_nulls} missing values with mode ({_perf_mode})")
    ...
    if 'JobInvolvement' in df.columns:
        ...
        _inv_mode = int(df['JobInvolvement'].dropna().mode().iloc[0]) if df['JobInvolvement'].notna().any() else 3
        df['JobInvolvement'] = df['JobInvolvement'].round(0).fillna(_inv_mode).astype(int)
        if ji_nulls > 0:
            cleaning_audit.append(f"JobInvolvement: Filled {ji_nulls} missing values with mode ({_inv_mode})")
    ...
    for col in median_cols:
        if col in df.columns:
            ...
            if missing > 0:
                cleaning_audit.append(f"{col}: Filled {missing} missing values with median ({_med:.1f})")
