...
def patch_behavior_engine():
    ...
    with (
        patch("backend.core.simulation.time_engine.update_agent_state"),
...
def patch_quit_model():
    ...
    with (
        patch("backend.core.simulation.time_engine._quit_model", return_value=mock_model),
        ...
        patch(
            "backend.core.ml.attrition_model.engineer_features",
...
class TestSimulationInvariants:
    ...
    def test_stress_amplification_override_zero_used(self):
        ...
        result_no_amp = run_simulation(
            config,
            agents=[_make_mock_agent(i, stress=0.6) for i in range(1, 11)],
            G=G,
            seed=42,
            stress_amplification_override=0.0,
        )
        ...
        result_with_amp = run_simulation(
            ...
            G=G,
            seed=42,
            stress_amplification_override=3.0,
        )

        # Amplification must increase attrition — if not, override isn't being applied
        assert (
            result_with_amp["summary"]["total_quits"] >= result_no_amp["summary"]["total_quits"]
        ), (
    ...
    def test_prob_scale_override_affects_attrition(self):
        ...
        result_low = run_simulation(
            config,
            agents=[_make_mock_agent(i) for i in range(1, 21)],
            G=G,
            seed=42,
            prob_scale_override=0.0001,
        )
