...
def orchestrate_user_request(user_text: str) -> dict:
    ...
    try:
        if api_key:
            ...
            response = client.chat.completions.create(
                model="llama-3.1-8b-instant", messages=messages, response_format={"type": "json_object"}, temperature=0.1
            )
            ...
            response = local_client.chat.completions.create(
                model="llama3.1:8b", messages=messages, response_format={"type": "json_object"}, temperature=0.1
            )
            intent_json = json.loads(response.choices[0].message.content)
    ...
    if intent_json.get("intent") == "chat":
        return {
            "type": "chat",
            "response": intent_json.get("chat_response", "I'm ready to simulate policies. What would you like to test?"),
        }
    ...
    print(f"[Orchestrator] Intent is simulate. Extracting parameters...")
    ...
    print(f"[Orchestrator] Generating Monte Carlo simulation...")
    runs = 10
    sim_result = run_simulation_job(
        policy_name="custom",
        runs=runs,
        policy_config=config.__dict__
    )
    ...
    print(f"[Orchestrator] Running Agent 2 Reasoning Chain...")
