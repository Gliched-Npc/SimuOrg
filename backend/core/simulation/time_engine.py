...
def run_simulation(
    ...
    _prob_scale = (
        prob_scale_override if prob_scale_override is not None else cal.get("prob_scale", 1.0)
    )
    ...
    if _policy_salary_pct > 0.0:
        ...
        print(
            f"[time_engine] Injecting salary hike: {_clamped_pct:.1f}% "
            ...
            for quitter in list(quitting_agents):
                if rng.random() < fill_prob:
                    ...
                    if _policy_salary_pct > 0.0:
                        ...
                        new_agent.monthly_income = new_agent.monthly_income * (
                            1.0 + _clamped_pct / 100.0
                        )
    ...
    # Summary
    total_quits = sum(log["attrition_count"] for log in logs)
    total_layoffs = sum(log["layoff_count"] for log in logs)
    final_headcount = logs[-1]["headcount"] if logs else initial_headcount
    period_months = config.duration_months

    avg_headcount = sum(log["headcount"] for log in logs) / len(logs) if logs else initial_headcount
    period_attrition_pct = (total_quits / avg_headcount * 100) if avg_headcount > 0 else 0.0
    annual_attrition_pct = (
        period_attrition_pct * (12.0 / period_months) if period_months > 0 else period_attrition_pct
    )

    total_workforce_loss = total_quits + total_layoffs
    total_workforce_loss_pct = (
        (total_workforce_loss / initial_headcount * 100) if initial_headcount > 0 else 0.0
    )
    ...
    print(
        f"   Attrition Rate    : {period_attrition_pct:.1f}% "
        f"(~{annual_attrition_pct:.1f}% annualised, voluntary only)"
    )
    print(f"   Workforce Loss    : {total_workforce_loss_pct:.1f}% " f"(voluntary + involuntary)")
