...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const stored = JSON.parse(
      localStorage.getItem("simuorg_job_history") || "[]",
    );
    Promise.all(
  ...
  const baseAttr =
    baseEmp > 0
      ? (employees.filter((e) => e.attrition === "Yes").length / baseEmp) * 100
      : 0;
  const baseSat =
    baseEmp > 0
      ? employees.reduce((s, e) => s + (e.job_satisfaction || 0), 0) / baseEmp
      : 0;

  const latestSim = jobHistory.find((j) => j.status === "completed");

  const totalEmpDisplay = latestSim?.headcount
    ? latestSim.headcount.toLocaleString()
    : loading
      ? "…"
      : baseEmp.toLocaleString();
  const attritionDisplay = latestSim?.attrition
    ? latestSim.attrition.toFixed(1)
    : loading
      ? "…"
      : baseAttr.toFixed(1);
  const avgSatDisplay = latestSim?.satisfaction
    ? latestSim.satisfaction.toFixed(2)
    : loading
      ? "…"
      : baseSat.toFixed(2);
    ...
    >
      ...
      <div style={{ marginBottom: "2.5rem" }}>
        ...
        <p className="page-sub">
          Organisational health at a glance (Showing Pre/Post Simulation
          Metrics)
        </p>
      </div>
      ...
      >
        ...
        <KpiCard
          ...
          delta={
            parseFloat(attritionDisplay) > 15
              ? "Above threshold"
              : "Within range"
          }
        />
        <KpiCard
          ...
          deltaDir={
            latestSim
              ? latestSim.satisfaction > baseSat
                ? "up"
                : "down"
              : undefined
          }
        ...
        <div className="glass-card" style={{ overflow: "hidden" }}>
          <div
            style={{
              ...
              borderBottom: "1px solid rgba(51,201,207,0.1)",
            }}
          >
            <div
              style={{
          ...
          {jobHistory.length === 0 ? (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(51,201,207,0.08)" }}>
                  {["Policy Prompt", "Status", "Health Score", "Time"].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
              ...
              <tbody>
                {jobHistory.map((job, i) => (
                  <tr
                    ...
                    onClick={() =>
                      navigate("/simulate", {
                        state: { jobId: job.job_id, prompt: job.prompt },
                      })
                    }
                    style={{
                      borderBottom: "1px solid rgba(51,201,207,0.06)",
                      cursor: "pointer",
            ...
            >
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <Server size={16} color="rgba(51,201,207,0.6)" />
                  <span
                    style={{
                      fontSize: "0.85rem",
                ...
                >
                  <span
                    style={{
                      ...
                      animation:
                        backendOk === null ? "pulse 1.4s infinite" : "none",
                    }}
                  />
                  {backendOk === null
                    ? "Checking"
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <Database size={16} color="rgba(51,201,207,0.6)" />
                  <span
                    style={{
                      fontSize: "0.85rem",
                      color: "rgba(255,255,255,0.7)",
              ...
              <div
                style={{
                  ...
                  border: "1px solid rgba(51,201,207,0.1)",
                }}
              >
                <div
                  style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    background: primary
                      ...
                      : "rgba(0,173,181,0.08)",
                    border: primary
                      ? "1px solid rgba(51,201,207,0.3)"
                      : "1px solid rgba(51,201,207,0.1)",
                    color: "#fff",
                    transition: "all 0.18s",
                    boxShadow: primary
                      ? "0 4px 16px rgba(0,173,181,0.25)"
                      : "none",
                  }}
                >
                  {label}
