...
def generate_extreme_dataset(filename="data_burnout_central.csv", num_samples=1000):
    ...
    for i in range(1, num_samples + 1):
        ...
        attrition_prob = 0.0
        if job_sat <= 2: attrition_prob += 0.3
        if wlb <= 2: attrition_prob += 0.3
        if overtime == "Yes": attrition_prob += 0.2
        if env_sat <= 2: attrition_prob += 0.1
...
def generate_chill_dataset(filename="data_chill_utopia.csv", num_samples=1000):
    ...
    for i in range(1, num_samples + 1):
        ...
        attrition_prob = 0.02
        if job_sat <= 2: attrition_prob += 0.1
        if wlb <= 2: attrition_prob += 0.1
        if overtime == "Yes": attrition_prob += 0.05
